        ...
        <q-table :rows="rows" :columns="columns" row-key="name" color="secondary" :pagination="initialPagination"
            ...
            <template v-slot:top>
                <q-toolbar class="p-none rounded-tl-lg  rounded-tr-lg" :glossy="true"
                    ...
                    <q-icon class="ml-3 p-1 rounded text-white bg-gradient-to-l from-lime-400 to-green-700" name="event_note"
                        size="30px" />
                    <q-toolbar-title><span class="mr-3 text-weight-medium">Lotes de Inscricões</span></q-toolbar-title>
                    <q-btn class="btn-scale m-2 pr-4 pl-2" color="primary" push glossy round label="Inserir"
                        v-if="geral.funcoesAcessos.loteInscricaoInserir" @click="insertRow" aria-label="Menu Sistema" icon="add">
            ...
            <template v-slot:body="props">
                <q-tr :props="props">
                    <!-- <q-td key="logo"> <q-img v-if="props.row.logo" class="rounded" style="width: 80px;" :ratio="1200 / 630"
                            :src="getUrlImagemThumb(props.row.logo)"></q-img></q-td> -->
                    <q-td key="descricao">{{ props.row.descricao }}</q-td>
                    <q-td key="inicio">{{ $geralService.getDataHoraFormatada(props.row.inicio) }}</q-td>
                    <q-td key="fim">{{ $geralService.getDataHoraFormatada(props.row.fim) }}</q-td>
                    <q-td key="action">
                        <q-btn btn-scale push round glossy icon="more_vert">
                            <q-menu>
                                <q-list separator link>
                                    ...
                                    <q-item v-if="geral.funcoesAcessos.loteInscricaoInserir" clickable v-close-popup
                                    ...
                                    <q-item clickable v-if="geral.funcoesAcessos.loteInscricaoDeletar" v-close-popup @click="deleteRow(props.rowIndex, props.row._id)">
                                        <q-item-section avatar>
...
import LoteInscricaoModal from "./loteInscricao/LoteInscricaoModal.vue";
...
const deleteRow = async (index, id) => {
    if (geral.funcoesAcessos.loteInscricaoDeletar) {
        $q.loading.show({
            spinner: QSpinnerOval,
            spinnerColor: "white",
            spinnerSize: 60,
        ...
        const ret = await useCustomFetch("loteInscricao/delete", "post", { _id: id, definitivo: false }, undefined);
        $q.loading.hide();
        if (ret.valido) {
            $q.notify({
                type: "positive",
...
const editRow = (index, id, copy) => {
    if (geral.funcoesAcessos.loteInscricaoEditar) {
        openModal(index, id, copy);
    }
};
const insertRow = () => {
    if (geral.funcoesAcessos.loteInscricaoInserir) {
        openModal(-1, null);
    }
};
