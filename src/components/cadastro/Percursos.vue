        ...
        <q-table :rows="rows" :columns="columns" row-key="name" color="secondary" :pagination="initialPagination"
            ...
            <template v-slot:top>
                <q-toolbar class="p-none rounded-tl-lg  rounded-tr-lg" :glossy="true"
                    ...
                    <q-icon class="ml-3 p-1 rounded text-white bg-gradient-to-l from-cyan-500 to-blue-700" name="trending_up"
                        size="30px" />
                    <q-toolbar-title><span class="mr-3 text-weight-medium">Percursos</span></q-toolbar-title>
                    <q-btn class="btn-scale m-2 pr-4 pl-2" color="primary" push glossy round label="Inserir"
                        v-if="geral.funcoesAcessos.percursoInserir" @click="insertRow" aria-label="Menu Sistema" icon="add">
            ...
            <template v-slot:body="props">
                <q-tr :props="props">
                    ...
                    <q-td key="modeloCategoria">{{ props.row.modeloCategoria.descricao }}</q-td>
                    <q-td key="action">
                        <q-btn btn-scale push round glossy icon="more_vert">
                            <q-menu>
                                <q-list separator link>
                                    ...
                                    <q-item v-if="geral.funcoesAcessos.percursoInserir" clickable v-close-popup
                                    ...
                                    <q-item v-if="geral.funcoesAcessos.percursoEditar" clickable v-close-popup
                                    ...
                                    <q-item clickable v-if="geral.funcoesAcessos.percursoDeletar" v-close-popup @click="deleteRow(props.rowIndex, props.row._id)">
                                        <q-item-section avatar>
...
import PercursoModal from "./PercursoModal.vue";
...
const columns = ref([
    ...
    },
    {
        ...
        required: true,
        label: "Modelo Categoria",
        align: "left",
        field: "modeloCategoria",
        sortable: true,
...
const deleteRow = async (index, id) => {
    if (geral.funcoesAcessos.percursoDeletar) {
        $q.loading.show({
            spinner: QSpinnerOval,
            spinnerColor: "white",
            spinnerSize: 60,
        ...
        const ret = await useCustomFetch("percurso/delete", "post", { _id: id, definitivo: false }, undefined);
        $q.loading.hide();
        if (ret.valido) {
            $q.notify({
                type: "positive",
...
const editRow = (index, id, copy) => {
    if (geral.funcoesAcessos.percursoEditar) {
        openModal(index, id, copy);
    }
};
const insertRow = () => {
    if (geral.funcoesAcessos.percursoInserir) {
        openModal(-1, null);
    }
};
