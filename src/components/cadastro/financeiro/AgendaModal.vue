    ...
    <q-dialog :persistent="true" :maximized="true" ref="dialogRef" @hide="onDialogHide">
      <q-card>
        ...
        <q-toolbar :glossy="true" :class="$q.dark.isActive ? 'text-grey-2 bg-gray-8' : 'bg-grey-2 text-gray-9'">
          <q-icon class="ml-3 p-1 rounded text-white bg-gradient-to-r from-indigo-700 to-sky-400" name="event" size="30px" />

          <q-toolbar-title>{{ inserir ? "Inserir - " : "Editar " }} Agenda</q-toolbar-title>
          <!-- GO btnSave -->

          <q-btn v-if="state.financeiroAgenda.status !== 'Concluído' && !inserir && state.financeiroAgenda.dataPagamento && state.financeiroAgenda.valorPago > 0 && (novoComp || state.financeiroAgenda.comprovanteImagem)" color="primary" @click="beforeSave(true)" aria-label="Pagar - " push glossy :round="$q.screen.xs ? true : false" class="btn-scale mr-3" :label="$q.screen.gt.xs ? 'Salvar e Concluir' : undefined" icon="event_available"></q-btn>
          <q-btn v-if="state.financeiroAgenda.status !== 'Cancelado' && !inserir" color="negative" @click="cancelarFinanceiroAgenda()" aria-label="Cancelar - " push glossy :round="$q.screen.xs ? true : false" class="btn-scale mr-3" :label="$q.screen.gt.xs ? 'Excluir Agenda' : undefined" icon="event_busy"></q-btn>
          <q-btn push glossy :round="$q.screen.xs ? true : false" class="btn-scale mr-3" color="positive" aria-label="Salvarmargin-extra - " @click="beforeSave(false)" :label="$q.screen.gt.xs ? 'Salvar' : undefined" icon="save"></q-btn>
        ...
        <q-card-section :style="{ 'max-height': 'calc(100vh - 50px)' }" class="scroll container mx-auto">
          <q-tabs class="" v-model="tab" :active-color="$q.dark.isActive ? 'info' : 'primary'" indicator-color="primary" align="justify" narrow-indicator>
          ...
          <q-tab-panels v-model="tab" animated>
            <q-tab-panel name="dados">
              <div class="q-ma-sm">
                <q-chip class="q-mb-md" :color="state.financeiroAgenda.status === 'Aberto' ? 'warning' : state.financeiroAgenda.status === 'Concluído' ? 'positive' : 'negative'" text-color="white" :icon="state.financeiroAgenda.status === 'Concluído' ? 'event_available' : state.financeiroAgenda.status === 'Aberto' ? 'event' : 'event_busy'">
                  ...
                  <div class="col-md-3 col-sm-6 col-12">
                    <q-field hide-bottom-space outlined label="Tipo" dense stack-label>
                      <template v-slot:control>
                        <div class="col-md-4 col-sm-6 col-12 self-center full-width no-outline row" tabindex="0">
                          <q-radio :readonly="state.financeiroAgenda.status === 'Concluído'" class="col-6" v-model="state.financeiroAgenda.tipo" val="D" label="Débito" />
                          <q-radio :readonly="state.financeiroAgenda.status === 'Concluído'" class="col-6" v-model="state.financeiroAgenda.tipo" val="C" label="Crédito" />
                        </div>
                      </template>
                    </q-field>
                  </div>
                  <div class="col-md-6 col-sm-6 col-12">
                    <q-input :readonly="state.financeiroAgenda.status === 'Concluído'" hide-bottom-space outlined v-model="state.financeiroAgenda.descricao" label="Descrição" :dense="dense" @blur="$v.financeiroAgenda.descricao.$touch" :error="$v.financeiroAgenda.descricao.$error" error-message="Campo obrigatório" />
                  </div>
                  <div class="col-md-3 col-sm-6 col-12">
                    <q-input :readonly="state.financeiroAgenda.status === 'Concluído'" hide-bottom-space type="number" outlined v-model="state.financeiroAgenda.valor" label="Valor Documento" :dense="dense" prefix="R$" @blur="$v.financeiroAgenda.valor.$touch" :error="$v.financeiroAgenda.valor.$error" error-message="Campo obrigatório" />
                  </div>
                ...
                <div class="row q-col-gutter-sm q-mt-sm">
                  <div class="col-md-2 col-sm-6 col-12">
                    <q-input :readonly="state.financeiroAgenda.status === 'Concluído'" hide-bottom-space outlined hint="ex. NF 0057" v-model="state.financeiroAgenda.documento" label="Documento" :dense="dense" />
                  </div>
                  <div class="col-md-3 col-sm-6 col-12">
                    <q-select :readonly="state.financeiroAgenda.status === 'Concluído'" hide-bottom-space outlined v-model="state.financeiroAgenda.conta" label="Conta" :dense="dense" option-label="descricao" :emit-value="true" map-options option-value="_id" :options="listaConta" @blur="$v.financeiroAgenda.conta.$touch" :error="$v.financeiroAgenda.conta.$error" error-message="Campo obrigatório">
                  ...
                  <div class="col-md-3 col-sm-6 col-12">
                    <q-select :readonly="state.financeiroAgenda.status === 'Concluído'" hide-bottom-space outlined v-model="state.financeiroAgenda.planoDeConta" :option-disable="(opt) => opt.especificacao === 'Sintética'" @update:model-value="inputPlanoConta" label="Plano de Conta" :dense="dense" map-options :option-label="getDescricaoPlanoConta" :emit-value="true" option-value="_id" :options="getListaPlanoFiltro" @blur="$v.financeiroAgenda.planoDeConta.$touch" :error="$v.financeiroAgenda.planoDeConta.$error" error-message="Campo obrigatório">
                      <template v-slot:append>
                        <q-icon name="search" />
                      </template>
                      <template v-slot:option="scope">
                        <q-item v-bind="scope.itemProps">
                          <q-item-section>
                            <q-item-label> {{ scope.opt.identificador }} - {{ scope.opt.descricao }} </q-item-label>
                          </q-item-section>
                        </q-item>
                      </template>
                    </q-select>
                  ...
                  <div class="col-md-2 col-sm-6 col-12">
                    <CinputDate :readonly="state.financeiroAgenda.status === 'Concluído'" :dark="$q.dark.isActive" outlined hide-bottom-space v-model="state.financeiroAgenda.dataVencimento" label="Dt. Vencimento" @blur="$v.financeiroAgenda.dataVencimento.$touch" :error="$v.financeiroAgenda.dataVencimento.$error" error-message="Campo obrigatório" :dense="dense" />
                  </div>
                  <div v-if="!inserir" class="col-md-2 col-sm-6 col-12">
                    <CinputDate :readonly="state.financeiroAgenda.status === 'Concluído'" :dark="$q.dark.isActive" outlined hide-bottom-space v-model="state.financeiroAgenda.dataPagamento" label="Dt. Pagamento" :dense="dense" />
                  </div>
                  <div v-if="!inserir" class="col-md-3 col-sm-6 col-12">
                    <q-input min="0" :readonly="state.financeiroAgenda.status === 'Concluído'" hide-bottom-space type="number" outlined v-model="state.financeiroAgenda.valorPago" label="Valor Pago" :dense="dense" prefix="R$" />
                  </div>
                </div>
              </div>
            </q-tab-panel>
            <q-tab-panel name="documento">
              <div class="row q-col-gutter-sm">
                <div class="col-12 col-sm-10 offset-sm-1 col-md-8 offset-md-2">
                  <q-file style="display: none" ref="filea" accept=".png, .jpg, .jpeg, .pdf" @update:model-value="onFileDoc" label="Documento" outlined v-model="novoDocumento" error-message="Clique ou arreste o arquivo no campo">
                  ...
                  <q-card inline class="q-ma-sm">
                    <q-card-section v-if="state.financeiroAgenda.status !== 'Concluído'">
                      <q-btn icon="attach_file" class="q-mr-sm" @click="filea.$el.click()" outline color="primary" label="Buscar" />
                      <q-btn
                        class="q-mr-sm"
                        ...
                        @click="
                          novoDoc = null;
                          novoDocumento = null;
                          state.financeiroAgenda.documentoImagem = '';
                        "
                        outline
                        color="secondary"
                        label="Ficar Sem"
                      ...
                      <q-btn
                        ...
                        @click="
                          novoDoc = null;
                          novoDocumento = null;
                          state.financeiroAgenda.documentoImagem = documentoPadrao;
                        "
                        outline
                        color="deep-purple"
                        label="Restaurar"
                    ...
                    <q-card-section>
                      <q-img v-if="!documentoIsPdf && (novoDoc || state.financeiroAgenda.documentoImagem)" :src="novoDoc ? novoDoc : $geralService.configuracoes.BASE_S3 + state.financeiroAgenda.documentoImagem" spinner-color="primary" spinner-size="82px"></q-img>
                      <div v-else>
                      </div>
                    </q-card-section>
                  </q-card>
            ...
            <q-tab-panel name="comprovante">
              <div class="row q-col-gutter-sm">
                <div class="col-12 col-sm-10 offset-sm-1 col-md-8 offset-md-2">
                  <q-file style="display: none" ref="fileb" accept=".png, .jpg, .jpeg, .pdf" @update:model-value="onFileComp" label="Documento" outlined v-model="novoComprovante" error-message="Clique ou arreste o arquivo no campo">
                    <template v-slot:prepend>
                      <q-icon class="cursor-pointer" name="attach_file" @click.stop />
                    </template>
                  </q-file>
                  <q-card inline class="q-ma-sm">
                    <q-card-section v-if="state.financeiroAgenda.status !== 'Concluído'">
                      ...
                      <q-btn
                        ...
                        @click="
                          ...
                          state.financeiroAgenda.comprovanteImagem = '';
                        "
                        outline
                        color="secondary"
                        label="Ficar Sem"
                      ...
                      <q-btn
                        ...
                        @click="
                          novoComp = null;
                          novoComprovante = null;
                          state.financeiroAgenda.comprovanteImagem = comprovantePadrao;
                        "
                        outline
                        color="deep-purple"
                        label="Restaurar"
                    ...
                    <q-card-section>
                      <q-img v-if="!comprovanteIsPdf && (novoComp || state.financeiroAgenda.comprovanteImagem)" :src="novoComp ? novoComp : $geralService.configuracoes.BASE_S3 + state.financeiroAgenda.comprovanteImagem" spinner-color="primary" spinner-size="82px"></q-img>
...
const visualizarPdf = async (caminho: any) => {
  $q.dialog({
...
const getDescricaoPlanoConta = (obj: any) => {
  return obj ? obj.identificador + " - " + obj.descricao : "";
};
...
const uploadDocumento = async (newFinanceiroAgenda: any) => {
  if (novoDocumento.value) {
    ...
    if (await $geralService.uploadFileAsync(novoDocumento.value, caminho, randonName, undefined)) {
