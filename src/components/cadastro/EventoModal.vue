    ...
    <q-dialog :persistent="true" :maximized="true" ref="dialogRef" @hide="onDialogHide">
      <q-card>
        ...
        <q-toolbar :glossy="true" :class="$q.dark.isActive ? 'text-grey-2 bg-gray-8' : 'bg-grey-2 text-gray-9'">
          <q-icon class="ml-3 p-1 rounded text-white bg-gradient-to-r from-lime-400 to-gray-600" name="event" size="30px" />
            ...
            <q-tab-panel name="dados">
              <div class="q-ma-sm">
                <div class="row q-col-gutter-sm">
                  ...
                  <!-- GO Ano -->
                  <div class="col-sm-2 col-12">
                    <q-select hide-bottom-space outlined v-model="state.evento.ano" label="Ano" :dense="dense" :options="listaAnos"> </q-select>
                  </div>
                  <!-- GO Etapa -->
                    ...
                    <q-field outlined>
                      <q-item tag="label">
                        <q-item-section avatar top>
                          <q-checkbox v-model="state.evento.cartao" />
                        </q-item-section>
                        <q-item-section>
                          <q-item-label>Permitir Pagamento com Cartão de Crédito</q-item-label>
                          <q-item-label caption>
                            {{ state.evento.cartao ? "Cartão e PIX serão aceitos. OBS: Os valores de cartão levam 30 dias para estarem disponíveis, PIX é imediato." : "Somente PIX será aceito!" }}
                          </q-item-label>
                        </q-item-section>
                      </q-item>
                    </q-field>
                  ...
                  <div class="row q-col-gutter-sm q-pa-md">
                    <div class="col-sm-5 col-12">
                      <q-field :readonly="index === 0" outlined label="Pessoa" stack-label :dense="false" :error="!adm.pessoa" error-message="Campo obrigatório">
                        ...
                        <template v-slot:append>
                          <q-btn color="primary" v-if="index > 0" @click="selecionarAdm(index)" icon="search" round>
              ...
              <q-list bordered class="rounded-borders az-radius-top">
                <q-list bordered class="rounded-lg">
                  <Expansion group="grupoGeral" icon="link" labelBotao="Inserir Grupo" corBotao="indigo" :erroPadrao="$v.evento.grupoLinks.$error" titulo="Grupos de Links" @onInserir="inserirGrupoLink">
                    <GrupoLink v-for="(item, index) in state.evento.grupoLinks" draggable="true" @dragend="(e: any) => finishDrag(state.evento.grupoLinks, item, index, e)" @dragstart="(e: any) => startDrag(item, index, e)" :key="'grupoLink' + index" v-model="state.evento.grupoLinks[index]" :id="state.evento._id" :index="index" :collectionName="'evento/' + state.evento.sigla + '/' + state.evento.ano" @onExcluirGrupoLink="excluirGrupoLink" />
                  </Expansion>
                  <Expansion group="grupoGeral" labelBotao="Inserir Conteudo" corBotao="indigo" icon="ballot" :erroPadrao="$v.evento.conteudos.$error" titulo="Conteúdos Extras" @onInserir="inserirConteudo">
                    <ConteudoExtra v-for="(item, index) in state.evento.conteudos" draggable="true" @dragend="(e: any) => finishDrag(state.evento.conteudos, item, index, e)" @dragstart="(e: any) => startDrag(item, index, e)" :key="'conteudo' + index" v-model="state.evento.conteudos[index]" @onExcluir="deletarGeral" />
                  </Expansion>
                  <Expansion group="grupoGeral" labelBotao="Inserir Contato" corBotao="indigo" icon="contacts" :erroPadrao="$v.evento.contatos.$error" titulo="Contatos" @onInserir="inserirContato">
                    <Contato v-for="(item, index) in state.evento.contatos" @dragover="(e: any) => onDragOver(item, index, e)" draggable="true" @dragend="(e: any) => finishDrag(state.evento.contatos, item, index, e)" @dragstart="(e: any) => startDrag(item, index, e)" :key="'contato' + index" v-model="state.evento.contatos[index]" :index="index" @onExcluir="deletarGeral" />
                  </Expansion>
                  <Expansion group="grupoGeral" labelBotao="Inserir" corBotao="indigo" icon="more_time" :erroPadrao="$v.evento.programacoes.$error" titulo="Programação" @onInserir="inserirProgramacoes">
                    <Programacao v-for="(item, index) in state.evento.programacoes" @dragover="(e: any) => onDragOver(item, index, e)" draggable="true" @dragend="(e: any) => finishDrag(state.evento.programacoes, item, index, e)" @dragstart="(e: any) => startDrag(item, index, e)" :key="'programacao' + index" v-model="state.evento.programacoes[index]" :index="index" @onExcluir="deletarGeral" />
                  </Expansion>

                  <q-expansion-item group="grupoGeral" header-class="az-radius-top" expand-separator icon="tag" label="Tags">
                    <div class="row q-col-gutter-sm p-4">
                      <div class="col-12">
                        <q-field outlined label="Tags" stack-label :dense="dense">
                          <template v-slot:control>
                            <div class="self-center full-width no-outline" tabindex="0">
                              <q-chip v-for="(tag, index) in state.evento.tags" :key="index" dense removable @remove="state.evento.tags.splice(index, 1)" color="primary" text-color="white">
                                {{ tag }}
                              </q-chip>
                            </div>
                          </template>
...
const beforeSave = () => {
  ...
  $geralService.executaUpload(newEvento, "imagem", imageData.value, inserir.value, "jpg", "eventos/imagens/", save, undefined);
};
