<template>
  <div>
    <q-table :id="uuid" :rows="getFilteredValuesData" :columns="columns" row-key="name" :class="classes" :visible-columns="visible_columns" :pagination="pagination" :separator="separator" :dense="dense" :dark="dark" :flat="flat" :bordered="bordered" :square="square" :selection="selection_prop" :selected.sync="selected_prop" :filter="filter">
      <template v-slot:top="props">
        <slot name="top" v-bind:props="props"> </slot>
      </template>
      ...
      <template v-slot:header="props">
        <q-tr :props="props">
          ...
          <!-- {{props}} -->
          <q-th :props="props" @hover.native.stop v-for="col in props.cols" :key="col.name" auto-width>
            <div class="row inline">
              ...
              <div class="column">
                <q-btn flat dense size="sm" icon="fa fa-filter" class="q-ml-xs" @click.stop v-if="header_filter">
                  <q-icon name="fas fa-asterisk" color="red" style="font-size: 7px" v-if="column_options_selected[col.field].length > 0"></q-icon>
                  <q-menu>
                    <q-space />

                    <q-btn dense class="float-right q-ma-sm bg-red text-white" round size="sm" v-close-popup flat icon="close" />

                    <div class="q-pa-sm q-mt-md">
                      <q-select map-options multiple emit-value filled v-model="column_options_selected[col.field]" :options="column_options[col.field]" style="width: 150px !important" />
                    </div>
                    <q-btn color="primary" class="float-right q-mr-sm q-mb-sm text-capitalize" size="sm" v-close-popup @click="column_options_selected[col.field] = []" label="Clear" />
                  </q-menu>
                </q-btn>
              </div>
            </div>
        ...
        <q-tr :props="props" class="ignore-elements" v-if="columns_filter">
          ...
          <q-th :key="col.name" v-for="col in props.cols" style="padding: 0px 0px 0px 0px">
            ...
            <q-select class="q-pl-xs q-pr-xs" v-if="col.hasOwnProperty('filter_type') && col.filter_type == 'select'" map-options emit-value outlined v-model="column_options_selected[col.field]" :options="col.column_options" dense>
      ...
      <template v-slot:top-right="props" v-if="excel_download || csv_download || fullscreen || global_search">
        ...
        <q-btn class="bg-grey-2 q-mr-sm" icon="fas fa-file-excel" no-caps v-if="excel_download" @click="exportTable('xlsx')" />

        <q-btn class="bg-primary text-white" icon="fas fa-file-csv" no-caps v-if="csv_download" @click="exportTable('csv')" />

        <q-btn v-if="fullscreen" flat round class="q-ml-sm" dense :icon="props.inFullscreen ? 'fullscreen_exit' : 'fullscreen'" @click="props.toggleFullscreen">
          <q-tooltip :readonly="$q.platform.is.mobile" v-close-popup>{{ props.inFullscreen ? "Exit Fullscreen" : "Toggle Fullscreen" }}</q-tooltip>
        </q-btn>
      </template>
      ...
      <template v-slot:body="props">
        ...
        <slot name="body" v-bind="props" v-if="hasDefaultSlot"> </slot>
      </template>
    </q-table>
  </div>
</template>
...
export default {
  ...
  computed: {
    getFilteredData() {
      ...
      const table_Data = this.data.filter(function (item) {
        ...
        for (i = 0; i < table_columns.length; i++) {
          ...
          try {
            const valorColuna = table_columns[i].split(".").reduce(function (object, property) {
    ...
    getFilteredValuesData() {
      ...
      const table_Data = this.getFilteredData.filter(function (item) {
        ...
        // console.log(item[self.columns[i].field].toString());
        return true;
      });
      // eslint-disable-next-line camelcase
      return table_Data;
  ...
  created() {
    ...
    self.columns.filter(function (column) {
      self.column_options[column.field] = [...new Map(self.column_options[column.field].map((item) => [item.value, item])).values()];
    });
  },
  methods: {
    exportTable(type) {
