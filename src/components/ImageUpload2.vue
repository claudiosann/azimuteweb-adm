    ...
    <q-dialog :persistent="true" :maximized="true" ref="dialogRef" @hide="onDialogHide">
        <q-card :style="{ 'max-height': '100vh' }">
            ...
            <q-toolbar :glossy="true" :class="$q.dark.isActive ? 'text-grey-2 bg-gray-8' : 'bg-grey-2 text-gray-9'">
                ...
                <!-- GO btnSave -->
                <q-btn :round="$q.screen.xs ? true : false" glossy push class="btn-scale" @click="onDialogCancel"
            ...
            <div v-if="image.src" :style="{ 'max-height': 'calc(100vh - 80   px)' }">
                ...
                <vue-cropper class="img-container" ref="cropper" :aspect-ratio="winit / hinit" :src="image.src"
                    :modal="true" :movable="true" dragMode="move" :center="true" preview=".preview" />
            ...
            <div class="row gap-2 full-width flex justify-center mt-2">
                ...
                <q-btn v-if="image.src" h-11 ref="botaoEscolher" push color="primary" label="Confirmar"
                    @click="confirmar()" />
...
const confirmar = () => {
    cropper.value.getCroppedCanvas({ width: props.winit, height: props.hinit }).toBlob((blob) => {
