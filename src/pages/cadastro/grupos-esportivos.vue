    ...
    <q-table :rows="rows" :columns="columns" row-key="name" color="secondary" :pagination="initialPagination"
      ...
      <template v-slot:top>
        <q-toolbar class="p-none rounded-tl-lg  rounded-tr-lg" :glossy="true"
          ...
          <q-icon class="ml-3 p-1 rounded text-white bg-gradient-to-r from-blue-400 to-blue-700" name="scatter_plot" size="30px" />
          <q-toolbar-title><span class="mr-3 text-weight-medium">Grupos Esportivos</span></q-toolbar-title>
          <q-btn class="btn-scale m-2 pr-4 pl-2" color="primary" push glossy round label="Inserir"
            v-if="geral.funcoesAcessos.grupoEsportivoInserir" @click="insertRow" aria-label="Menu Sistema" icon="add">
      ...
      <template v-slot:body="props">
        <q-tr :props="props">
          ...
          <q-td key="caracteristica"> <div class="truncate">{{ props.row.caracteristica }}</div></q-td>
            ...
            <q-btn btn-scale push round glossy icon="more_vert">
              <q-menu>
                <q-list separator link>
                  ...
                  <q-item v-if="geral.funcoesAcessos.grupoEsportivoInserir" clickable v-close-popup
...
import GrupoEsportivoModal from "../../components/cadastro/GrupoEsportivoModal.vue";
...
const deleteRow = async (index, id) => {
  if (geral.funcoesAcessos.grupoEsportivoDeletar) {
    $q.loading.show({
      spinner: QSpinnerOval,
      spinnerColor: "white",
      spinnerSize: 60,
    ...
    const ret = await useCustomFetch("grupoEsportivo/delete", "post", { _id: id }, undefined);
    $q.loading.hide();
    if (ret.valido) {
      $q.notify({
        type: "positive",
...
const editRow = (index, id, copy) => {
  if (geral.verificaAcesso("grupoEsportivoEditar")) {
    openModal(index, id, copy);
  }
};
const insertRow = () => {
  if (geral.verificaAcesso("grupoEsportivoInserir")) {
    openModal(-1, null);
  }
};
