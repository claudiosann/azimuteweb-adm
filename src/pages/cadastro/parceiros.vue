        ...
        <q-table :rows="rows" :columns="columns" row-key="name" color="secondary" :pagination="initialPagination"
            ...
            <template v-slot:top>
                <q-toolbar class="p-none rounded-tl-lg  rounded-tr-lg" :glossy="true"
                    ...
                    <q-icon class="ml-3 p-1 rounded text-white bg-gradient-to-r from-red-500 to-red-800"
                        name="fas fa-handshake" size="30px" />
                    <q-toolbar-title><span class="mr-3 text-weight-medium">Parceiros</span></q-toolbar-title>
                    <q-btn class="btn-scale m-2 pr-4 pl-2" color="primary" push glossy round label="Inserir"
                        v-if="geral.funcoesAcessos.parceiroInserir" @click="insertRow" aria-label="Menu Sistema" icon="add">
            ...
            <template v-slot:body="props">
                <q-tr :props="props">
                    ...
                    <q-td key="site"> {{ props.row.site }} </q-td>
                        ...
                        <q-btn btn-scale push round glossy icon="more_vert">
                            <q-menu>
                                <q-list separator link>
                                    ...
                                    <q-item v-if="geral.funcoesAcessos.parceiroInserir" clickable v-close-popup
...
import ParceiroModal from "../../components/cadastro/ParceiroModal.vue";
...
const deleteRow = async (index, id) => {
    if (geral.funcoesAcessos.parceiroDeletar) {
        $q.loading.show({
            spinner: QSpinnerOval,
            spinnerColor: "white",
            spinnerSize: 60,
        ...
        const ret = await useCustomFetch("parceiro/delete", "post", { _id: id }, undefined);
        $q.loading.hide();
        if (ret.valido) {
            $q.notify({
                type: "positive",
...
const editRow = (index, id, copy) => {
    if (geral.verificaAcesso("parceiroEditar")) {
        openModal(index, id, copy);
    }
};
const insertRow = () => {
    if (geral.verificaAcesso("parceiroInserir")) {
        openModal(-1, null);
    }
};
