    ...
    <q-grid v-if="rows" :data="rows" :columns="columns" :columns_filter="true">
      <template v-slot:top>
        <q-toolbar class="p-none rounded-tl-lg rounded-tr-lg" :glossy="true" :class="$q.dark.isActive ? 'text-grey-2 bg-gray-8' : 'bg-grey-2 text-gray-9'">
          <q-icon class="ml-3 p-1 rounded text-white bg-gradient-to-r from-lime-400 to-gray-600" name="fas fa-calendar-alt" size="30px" />
          <q-toolbar-title><span class="mr-3 text-weight-medium">Eventos</span></q-toolbar-title>
          <q-btn class="btn-scale m-2 pr-4 pl-2" color="primary" push glossy round label="Inserir" v-if="geral.funcoesAcessos.eventoInserir" @click="insertRow" aria-label="Menu Sistema" icon="add">
      ...
      <template v-slot:body="props">
        <q-tr :props="props">
          ...
          <q-td key="nome">
            ...
            <q-item>
                ...
                <q-item-section>
                    ...
                    <q-item-label caption lines="2">{{ props.row.etapa }}</q-item-label>
                </q-item-section>
            </q-item>
          ...
          <q-td key="action">
            <q-btn round push glossy btn-scale icon="settings" @click="$router.push('/cadastro/eventos/' + props.row._id)"> </q-btn>
          </q-td>
        </q-tr>
      </template>
    </q-grid>
...
const columns = ref([
  ...
  },
  {
    ...
    sortable: true,
    filter_type: "select",
    column_options: ["Ativo", "Inativo", "Cancelado", "Finalizado"],
  },
  {
    name: "endereco.uf",
    required: true,
...
const getList = async () => {
  ...
  try {
    ...
    const ret = await useCustomFetch(
      ...
      "post",
      {
        filtro: { lixo: false, "administradores.pessoa": geral.pessoa._id === "5aff4d2f47667633c7ace227" ? undefined : geral.pessoa._id },
      },
      undefined
    );
