        ...
        <q-table :rows="rows" :columns="columns" row-key="name" color="secondary" :pagination="initialPagination"
            ...
            <template v-slot:top>
                <q-toolbar class="p-none rounded-tl-lg  rounded-tr-lg" :glossy="true"
                    ...
                    <q-icon class="ml-3 text-white p-1 rounded bg-gradient-to-r from-teal-500 to-teal-800"
                        name="category" size="30px" />
                    <q-toolbar-title><span class="mr-3 text-weight-medium">Modelos de Categorias</span></q-toolbar-title>
                    <q-btn class="btn-scale m-2 pr-4 pl-2" color="primary" push glossy round label="Inserir"
                        v-if="geral.funcoesAcessos.modeloCategoriaInserir" @click="insertRow" aria-label="Menu Sistema"
                        icon="add">
                        ...
                        <q-btn btn-scale push round glossy icon="more_vert">
                            <q-menu>
                                <q-list separator link>
                                    ...
                                    <q-item v-if="geral.funcoesAcessos.modeloCategoriaInserir" clickable v-close-popup
...
const deleteRow = async (index, id) => {
    if (geral.funcoesAcessos.modeloCategoriaDeletar) {
        $q.loading.show({
            spinner: QSpinnerOval,
            spinnerColor: "white",
            spinnerSize: 60,
        ...
        const ret = await useCustomFetch("modeloCategoria/delete", "post", { _id: id }, undefined);
        $q.loading.hide();
        if (ret.valido) {
            $q.notify({
                type: "positive",
...
const editRow = (index, id, copy) => {
    if (geral.verificaAcesso("modeloCategoriaEditar")) {
        openModal(index, id, copy);
    }
};
const insertRow = () => {
    if (geral.verificaAcesso("modeloCategoriaInserir")) {
        openModal(-1, null);
    }
};
...
const openModal = (index, id, copy) => {
    ...
    $q.dialog({
        component: ModeloCategoriaModal,
        persistent: true,
        componentProps: {
            copia: copy,
