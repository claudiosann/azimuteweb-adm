    ...
    <q-table :rows="rows" :columns="columns" row-key="name" color="secondary" :pagination="initialPagination"
      ...
      <template v-slot:top>
        <q-toolbar class="p-none rounded-tl-lg  rounded-tr-lg" :glossy="true"
          ...
          <q-icon class="ml-3 p-1 rounded text-white bg-gradient-to-r from-orange-700 to-yellow-400" name="account_balance"
            size="30px" />
          <q-toolbar-title><span class="mr-3 text-weight-medium">Contas</span></q-toolbar-title>
          <q-btn class="btn-scale m-2 pr-4 pl-2" color="primary" push glossy round label="Inserir"
            v-if="geral.funcoesAcessos.financeiroContaInserir" @click="insertRow" aria-label="Menu Sistema" icon="add">
      ...
      <template v-slot:body="props">
        <q-tr :props="props">
           <q-td key="descricao">{{ props.row.descricao }}</q-td>
              ...
              <q-td key="saldoAtual">{{ props.row.saldoAtual }}</q-td>
            ...
            <q-btn btn-scale push round glossy icon="more_vert">
              <q-menu>
                <q-list separator link>
                  ...
                  <q-item v-if="geral.funcoesAcessos.financeiroContaInserir" clickable v-close-popup
...
import ContaModal from "@/components/cadastro/financeiro/ContaModal.vue";
...
const deleteRow = async (index, id) => {
  if (geral.funcoesAcessos.financeiroContaDeletar) {
    $q.loading.show({
      spinner: QSpinnerOval,
      spinnerColor: "white",
      spinnerSize: 60,
    ...
    const ret = await useCustomFetch("financeiroConta/delete", "post", { _id: id }, undefined);
    $q.loading.hide();
    if (ret.valido) {
      $q.notify({
        type: "positive",
...
const editRow = (index, id, copy) => {
  if (geral.verificaAcesso("financeiroContaEditar")) {
    openModal(index, id, copy);
  }
};
const insertRow = () => {
  if (geral.verificaAcesso("financeiroContaInserir")) {
    openModal(-1, null);
  }
};
...
const openModal = async (index, id, copy) => {
  ...
  let movimentado = false;
  if(id){
    const movimento = await useCustomFetch('financeiroMovimentacao/getPopulate', 'post', { filtro: { lixo: false, conta: id }, limit: 1, select:{_id: 1} }, undefined);
    if (movimento.valido) {
      if (movimento.data.length > 0) {
        $q.notify({
          ...
          position: 'top',
          icon: 'warning',
          message: 'Atenção ao editar, conta possui lançamentos de movimentação!'
        });
        movimentado = true;
      }
    }
