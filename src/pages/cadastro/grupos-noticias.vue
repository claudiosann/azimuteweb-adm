        ...
        <q-table :rows="rows" :columns="columns" row-key="name" color="secondary" :pagination="initialPagination"
            ...
            <template v-slot:top>
                <q-toolbar class="p-none rounded-tl-lg  rounded-tr-lg" :glossy="true"
                    ...
                    <q-icon class="ml-3 bg-gradient-to-r from-rose-700 to-pink-600 text-white p-1 rounded" name="fa-solid fa-newspaper" size="30px" />
                    <q-toolbar-title><span class="mr-3 text-weight-medium">Grupos de Notícias</span></q-toolbar-title>
                    <q-btn class="btn-scale m-2 pr-4 pl-2" color="primary" push glossy round label="Inserir"
                        v-if="geral.funcoesAcessos.noticiaGrupoInserir" @click="insertRow" aria-label="Menu Sistema"
                        icon="add">
            ...
            <template v-slot:body="props">
                <q-tr :props="props">
                    ...
                    <q-td key="cor">
                        <div class="truncate">
                            <div class="p-1 rounded-md text-white font-bold" :style="{background:  props.row.cor}">{{ props.row.cor }}</div>
                        ...
                        <q-btn btn-scale push round glossy icon="more_vert">
                            <q-menu>
                                <q-list separator link>
                                    ...
                                    <q-item v-if="geral.funcoesAcessos.noticiaGrupoInserir" clickable v-close-popup
...
import GrupoNoticiaModal from "../../components/cadastro/GrupoNoticiaModal.vue";
...
const deleteRow = async (index, id) => {
    if (geral.funcoesAcessos.noticiaGrupoDeletar) {
        $q.loading.show({
            spinner: QSpinnerOval,
            spinnerColor: "white",
            spinnerSize: 60,
        ...
        const ret = await useCustomFetch("noticiaGrupo/delete", "post", { _id: id }, undefined);
        $q.loading.hide();
        if (ret.valido) {
            $q.notify({
                type: "positive",
...
const editRow = (index, id, copy) => {
    if (geral.verificaAcesso("noticiaGrupoEditar")) {
        openModal(index, id, copy);
    }
};
const insertRow = () => {
    if (geral.verificaAcesso("noticiaGrupoInserir")) {
        openModal(-1, null);
    }
};
