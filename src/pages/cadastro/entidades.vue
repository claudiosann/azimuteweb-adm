    ...
    <q-table :rows="rows" :columns="columns" row-key="name" color="secondary" :pagination="initialPagination"
      ...
      <template v-slot:top>
        <q-toolbar class="p-none rounded-tl-lg  rounded-tr-lg" :glossy="true"
          ...
          <q-icon size="30px" class="ml-3 p-1  rounded text-white bg-gradient-to-l from-sky-500  to-sky-700" name="home_work" />
          <q-toolbar-title><span class="mr-3 text-weight-medium">Entidades</span></q-toolbar-title>
          <q-btn class="btn-scale m-2 pr-4 pl-2" color="primary" push glossy round label="Inserir"
            v-if="geral.funcoesAcessos.entidadeInserir" @click="insertRow" aria-label="Menu Sistema" icon="add">
      ...
      <template v-slot:body="props">
        <q-tr :props="props">
           <q-td key="nomeRazao"> <q-img v-if="props.row.logo" class="rounded-borders" style="width: 48px;" :ratio="48 / 48" :src="getUrlImagemThumb(props.row.logo)"></q-img> {{ props.row.nomeRazao }} </q-td>
              <q-td key="nomeFantasia"> {{ props.row.nomeFantasia }} </q-td>
              <q-td key="tipo">{{ props.row.tipo }}</q-td>
              <q-td key="cnpj">{{ props.row.cnpj }}</q-td>
            <q-td key="action">
...
import EntidadeModal from "../../components/cadastro/EntidadeModal.vue";
