<template>
    <div v-if="rows" class="container mx-auto p-1 sm:p-4">
        <q-table :rows="rows" :columns="columns" row-key="name" color="secondary" :pagination="initialPagination"
            ...
            <template v-slot:top>
                <q-toolbar class="p-none rounded-tl-lg  rounded-tr-lg" :glossy="true"
                    ...
                    <q-icon class="ml-3 p-1 rounded text-white bg-gradient-to-r from-yellow-600 to-red-600" name="sports_kabaddi"
                        size="30px" />
                    <q-toolbar-title><span class="mr-3  text-weight-medium">Modalidades Esportivas</span></q-toolbar-title>
                    <q-btn class="btn-scale m-2 pr-4 pl-2" color="primary" push glossy round label="Inserir"
                        v-if="geral.funcoesAcessos.modalidadeEsportivaInserir" @click="insertRow" aria-label="Menu Sistema" icon="add">
            ...
            <template v-slot:body="props">
                <q-tr :props="props">
                    ...
                    <q-td key="esporte">{{ props.row.esporte?props.row.esporte.descricao:"" }}</q-td>
                        ...
                        <q-btn btn-scale push round glossy icon="more_vert">
                            <q-menu>
                                <q-list separator link>
                                    ...
                                    <q-item v-if="geral.funcoesAcessos.modalidadeEsportivaInserir" clickable v-close-popup
    ...
    try {
        const ret = await useCustomFetch(
            ...
            "post",
            {
                ...
                populateObj: [{path: 'esporte', select: {descricao: 1}}],
            },
            undefined
        );
...
const deleteRow = async (index, id) => {
    if (geral.funcoesAcessos.modalidadeEsportivaDeletar) {
        $q.loading.show({
            spinner: QSpinnerOval,
            spinnerColor: "white",
            spinnerSize: 60,
        ...
        const ret = await useCustomFetch("modalidadeEsportiva/delete", "post", { _id: id, definitivo: false }, undefined);
        $q.loading.hide();
        if (ret.valido) {
            $q.notify({
                type: "positive",
...
const editRow = (index, id, copy) => {
    if (geral.verificaAcesso("modalidadeEsportivaEditar")) {
        openModal(index, id, copy);
    }
};
const insertRow = () => {
    if (geral.verificaAcesso("modalidadeEsportivaInserir")) {
        openModal(-1, null);
    }
};
...
const openModal = (index, id, copy) => {
    ...
    $q.dialog({
        component: ModalidadeEsportivaModal,
        persistent: true,
        componentProps: {
            copia: copy,
